...
export default function About(){
    const [activeAboutView, setActiveAboutView] = useState<'paragraph' | 'tree'>('paragraph');
            ...
            >
              ...
              <div className="flex justify-center items-center gap-4 mb-12">
                <div className="bg-white dark:bg-zinc-800 p-1.5 rounded-full shadow-lg border border-gray-100 dark:border-gray-700 flex relative">
                  <div 
                    className={`absolute inset-y-1.5 w-[calc(50%-6px)] bg-blue-500 rounded-full transition-all duration-300 ease-in-out ${activeAboutView === 'paragraph' ? 'left-1.5' : 'left-[calc(50%+1.5px)]'}`}
                  ></div>
                  <button 
                    onClick={() => setActiveAboutView('paragraph')}
                    className={`relative z-10 px-6 py-2 rounded-full text-sm font-medium transition-colors duration-300 w-32 ${activeAboutView === 'paragraph' ? 'text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                  >
                    <MenuBook />
                  </button>
                  <button 
                    onClick={() => setActiveAboutView('tree')}
                    className={`relative z-10 px-6 py-2 rounded-full text-sm font-medium transition-colors duration-300 w-32 ${activeAboutView === 'tree' ? 'text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                  >
                    <Timeline />
                  </button>
                </div>
                  ...
                  {activeAboutView === 'paragraph' ? (
                    ...
                    >
                     <p className="text-lg lg:text-xl text-gray-700 dark:text-gray-300 leading-relaxed font-medium mb-8">
                        I have been passionate by many things througout my life, but programming has always been the one i had 
                        the most satisfaction with. When i started learning by myself, I instantly knew I was in the right path and I 
                        wanted to base a career on web development. Ever since, I have been learning and improving my skills, 
                        and I'm probably still doing as you read this. 
                        <br />
                        I believe having a passion as a job is the best way to 
                        create quality products.
                        <br />
                        Aside from that, I enjoy cars, video games (playing and making), electronics and photography. All of these are passions of mine, that I mix together to create something that represents me and that I enjoy doing.
                      </p>

                      <div className="w-full max-w-lg border-l-4 border-blue-500/30 dark:border-blue-400/30 pl-6 py-2 italic">
                        <p className="text-xl text-gray-600 dark:text-gray-300 font-serif">"A jack of all trades is a master of none, but oftentimes better than a master of one"</p>
                        <p className="text-right text-sm font-bold mt-2 text-gray-500 dark:text-gray-400 uppercase tracking-wider">— William Shakespeare</p>
                      </div>
                    </motion.div>
                  ) : (
                    <motion.div
                      ...
                      <div className="space-y-8">
                        {[
                          { 
                            ...
                            title: 'Master\'s Degree in Full-Stack Development and Technical Leadership', 
                            place: '.decode', 
                          ...
                          { 
                            year: 'July 2025', 
                            title: 'Bachelor\'s Degree in Digital Project Management, Specialization in Web Development', 
                            place: 'EEMI', 
                          ...
                          { 
                            year: '2022', 
                            title: 'General Baccalaureate (French high school diploma)', 
                          ...
                          <motion.div 
                            ...
                            viewport={{ once: true }}
                            className={`flex items-center justify-between md:justify-center w-full group ${index % 2 === 0 ? 'flex-row-reverse md:flex-row' : ''}`}
                          >
                            <div className="w-full md:w-5/12 hidden md:block text-right pr-8">
                              {index % 2 === 0 && (
                                <div>
                                  <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1">{item.title}</h3>
                                  <div className="text-sm font-medium text-blue-500 mb-2">{item.place}</div>
                            ...
                            <div className="absolute left-4 md:left-1/2 transform -translate-x-1/2 w-4 h-4 bg-blue-500 rounded-full border-4 border-white dark:border-zinc-900 shadow-sm z-10 group-hover:scale-125 transition-transform duration-300"></div>

                            <div className="w-[calc(100%-40px)] md:w-5/12 pl-12 md:pl-8 text-left">
                              <div className="md:hidden text-2xl font-black text-blue-100 dark:text-zinc-800 mb-2">{item.year}</div>
                              {index % 2 !== 0 ? (
                              ...
                              {index % 2 === 0 && (
                                <div className="hidden md:block text-4xl font-black text-gray-300 dark:text-zinc-800 tracking-tighter transition-colors duration-300 group-hover:text-blue-200 dark:group-hover:text-zinc-700/50">
                                  {item.year}
                                </div>
                              )}
                            </div>
